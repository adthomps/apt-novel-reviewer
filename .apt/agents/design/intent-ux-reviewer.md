...
model_tier: standard
autonomy: advisory
escalation: Escalate a surface that blocks the primary task with no workaround, and any accessibility gap that would exclude a class of users, to the accountable human.
status: active
owner: APT
...
- First agent in the `design` domain; pairs with `agents/customer/*` for role-specific journeys.
