...
APT Agent or `apt-agent-standards` owns concrete agent catalogs, tool-specific agent files, role routers, and handoff ledgers. `apt-principles` defines what accountable orchestration requires and should reference implementation catalogs instead of copying them.
