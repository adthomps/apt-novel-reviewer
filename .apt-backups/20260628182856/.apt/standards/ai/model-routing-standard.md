...
APT Agent or `apt-agent-standards` owns concrete model registries, local model detection, provider mappings, and routing validation scripts. `apt-principles` defines capability-based routing rules and must avoid vendor- or model-name-specific runtime policy.
