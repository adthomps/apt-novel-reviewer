...
APT Agent or `apt-agent-standards` may implement routers, task packets, lifecycle commands, reports, and tool-native agent files. `apt-principles` must not contain that runtime implementation; it should only define the harness stages, required evidence, approval expectations, and review criteria.
